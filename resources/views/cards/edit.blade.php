<x-layouts.dashboard>
    <x-slot name="seo">
        <title>Edit Card - MemFlash</title>
        <meta name="description" content="Edit your flashcard">
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="mb-6 sm:mb-8">
            <div class="flex items-center mb-4">
                <a href="{{ route('decks.show', $card->deck) }}" class="text-gray-500 hover:text-gray-700 mr-4">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                    </svg>
                </a>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Edit Card</h1>
            </div>
            <p class="text-gray-600">Edit this card from "{{ $card->deck->name }}".</p>
        </div>
        ...
        <div class="bg-white shadow-sm rounded-xl border border-gray-100">
            <form method="POST" action="{{ route('cards.update', $card) }}" class="p-4 sm:p-6">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4 sm:mb-6">
                    ...
                    <textarea id="front"
                              ...
                              placeholder="Enter the front text (e.g., English word, question, or term)">{{ old('front', $card->front) }}</textarea>
                    @error('front')
                ...
                <div class="mb-4 sm:mb-6">
                    ...
                    <textarea id="back"
                              ...
                              placeholder="Enter the back text (e.g., translation, definition, or answer)">{{ old('back', $card->back) }}</textarea>
                    @error('back')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4 sm:mb-6 p-4 bg-gray-50 rounded-lg">
                    <h3 class="text-sm font-semibold text-gray-900 mb-3">Card Information</h3>
                    <div class="grid grid-cols-2 gap-4 text-center">
                        <div>
                            <p class="text-2xl font-bold text-primary-600">{{ $card->deck->cards()->count() }}</p>
                            <p class="text-xs text-gray-500">Total Cards</p>
                        </div>
                        <div>
                            <p class="text-2xl font-bold text-blue-600">{{ $card->created_at->diffForHumans() }}</p>
                ...
                <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
                    <a href="{{ route('decks.show', $card->deck) }}"
