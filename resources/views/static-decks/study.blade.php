            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 sm:gap-6">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 sm:gap-3 mb-2">
                        ...
                        <div class="min-w-0 flex-1">
                            <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 truncate">{{ $staticDeck->name }}</h1>
                    ...
                    <div class="flex gap-3 sm:gap-4">
                        <div class="bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 shadow-lg border border-white/20">
                            <div class="text-center">
                                <div class="text-xl sm:text-2xl font-bold text-blue-600" id="cards-remaining">{{ $dueCards->count() }}</div>
                ...
                <div class="flex gap-3 sm:hidden justify-center">
                    <div class="bg-white/80 backdrop-blur-sm rounded-xl p-3 shadow-lg border border-white/20">
                        <div class="text-center">
                            <div class="text-xl font-bold text-blue-600" id="cards-remaining-mobile">{{ $dueCards->count() }}</div>
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
            <div class="bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg border border-white/20">
                <div class="flex items-center justify-between mb-3 sm:mb-4">
                    ...
                    <span class="text-xs sm:text-sm text-gray-600" id="session-stats">0 completed</span>
                </div>
                <div class="relative">
                    ...
                    <div class="flex justify-between text-xs sm:text-sm text-gray-600 mt-2">
                        <span id="progress-text">0 / {{ $cardsPerDay ?? $dueCards->count() }}</span>
                ...
                <div class="bg-white/90 backdrop-blur-sm rounded-xl sm:rounded-2xl lg:rounded-3xl shadow-2xl border border-white/20 overflow-hidden" id="study-card">
                    <div class="text-center p-3 sm:p-4 lg:p-6 xl:p-8 2xl:p-12" style="min-height: 350px; sm:min-height: 400px; lg:min-height: 500px;">
                        ...
                        <div id="session-complete" class="hidden py-12 sm:py-16 lg:py-20">
                            ...
                            <h2 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 mb-3 sm:mb-4">Daily Goal Complete!</h2>
                            <p class="text-lg sm:text-xl text-gray-600 mb-6 sm:mb-8">You've completed your {{ $cardsPerDay ?? 10 }} cards for today. Great job! Come back tomorrow for more learning.</p>
...
@push('scripts')
<script>
class StaticDeckStudySession {
    constructor() {
        this.cards = @json($dueCards->toArray() ?? []);
    ...
    async init() {
        try {
            console.log('Initializing static deck study session...');
            ...
            console.log('Static deck study session initialized successfully');
        } catch (error) {
    ...
    async rateCard(quality) {
        ...
        // Check if daily limit is reached or session is complete
        const totalCards = {{ $cardsPerDay ?? $dueCards->count() ?? 0 }};
        const completedCards = totalCards - this.cards.length;
        
        if (completedCards >= totalCards || this.cards.length === 0) {
    ...
    async sendUpdates() {
        ...
        try {
            const response = await fetch('/api/static-study/batch-update', {
                method: 'POST',
                headers: {
    ...
    async restartSession() {
        ...
        try {
            // Reload the page to get fresh cards
